...
public class Metrics {
	...
	public static void locAddedAndChurn(String[] changes, List<File> files, List<File> allFiles) {
		...
		/*creating a clone of the file in order to be able to 
		 *retrieve the index of the previous file.*/
	...
	public static void changeSet(List<File> files, List<File> allFiles) {
		...
		for (int i=0; i< files.size(); i++) {
			...
			if (index != -1) {
				...
				float actualSum= chgSet+prevFile.getChgSetSize();
				file.setChgSetSize(actualSum);
	 ...
	 * @param allFiles, list of all the files in the revisions.
	 */
	public static void nAuthors(String author, List<File> files, List<File> allFiles) {
		...
		for (int i=0; i< files.size(); i++) {
			...
			if (index != -1) {
				File prevFile = allFiles.get(index);
				List<String> authors = prevFile.getAuthors();
				file.setnAuthors(authors.size());
				if (author!=null && !authors.contains(author)) {
					authors.add(author);
					file.setnAuthors(authors.size());
				}
				file.setAuthors(authors);
			}
			else {
